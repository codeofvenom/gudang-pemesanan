...
class Client extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->session->userdata('client_login') == true) {
            redirect('client/client/edit_profilku');
        } else {
    ...
    public function edit_profilku()
    {
        if ($this->session->userdata('client_login') == true) {
            ...
            $x['data'] = $this->Client_model->get_profilku($adm);
            $x['online'] = $this->Deskripsi_model->get_online();
            $this->load->view('client/Edit_Profilku', $x);
        } else {
            $this->load->view('system/login');
        }
    }
        ...
        if (!empty($_FILES['filefoto']['name'])) {
            if ($this->upload->do_upload('filefoto')) {
                ...
                if (empty($password) && empty($konfirm_password)) {
                    $this->Client_model->update_profil_client_tanpa_pass($kode, $nama, $username, $gambar);
                    ...
                    $this->Client_model->update_profil_client($kode, $nama, $username, $password, $gambar);
                    echo $this->session->set_flashdata('msg', 'info');
                    redirect('client/client/edit_profilku');
                }
            } else {
            ...
            $status = $this->input->post('xstatus');
            if (empty($password) && empty($konfirm_password)) {
                $this->Client_model->update_profil_client_tanpa_pass_gambar($kode, $nama, $username);
                echo $this->session->set_flashdata('msg', 'info');
                redirect('client/client/edit_profilku');
            } elseif ($password != $konfirm_password) {
                echo $this->session->set_flashdata('msg', 'error');
                ...
                $this->Client_model->update_profil_client_tanpa_gambar($kode, $nama, $username, $password);
                echo $this->session->set_flashdata('msg', 'warning');
                redirect('client/client/edit_profilku');
            }
        }
